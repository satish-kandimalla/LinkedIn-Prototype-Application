...
exports.validateUser = function(req, res) {
	...
	if ((pwd !== undefined && email !== undefined) &&
			(pwd !== "" && email !== "")) 
	{
		var msg_payload={"email": email,"password": password,"login_message":"signin"};
				...
				if(results.login ==="Success"){
					console.log("valid Login");
					 //res.send({"userId":results.userId,"lastLogin":results.lastLogin,"emailId":results.emailId,"login":"Success"});	  
					    localSession.user_id = results.userId;
						localSession.lastLogin = results.lastLogin;
						localSession.first_name = results.first_name;
						localSession.last_name = results.last_name;
						localSession.summary = results.summary;
						localSession.experience=results.experience;
						localSession.connections=results.connections;
						localSession.education=results.education;
						localSession.skills=results.skills;
						localSession.invitations=results.invitations;
						localSession.invite_first_name=results.invite_first_name;
						localSession.invite_last_name=results.invite_last_name;
...
exports.insertSummary = function(req, res) {
	...
	if ((summary !== undefined && userId !== undefined) &&
			(summary !== "" && userId !== "")) 
	{
		var msg_payload={"summary": summary, "userId": userId ,"profile_message":"insertSummary"};
		mq_client.make_request('profile_queue',msg_payload, function(err,results){
			if(err){
				...
				{
					console.log("In RMQC: Got return call from RMQ for InsertSummary"+ JSON.stringify(results));	
					if(results.res.insertSummary ==="Success"){
						res.send({"Status" : "Success"});
 						localSession.summary = results.session.summary;
					}
					else
						{
